...
run_simulation <- function(t, timeframe, I, A, parms) {
  ...
  while (t < timeframe) {
    ...
    total_rate <- sum(i_plant_recovery, 
                      #aphid_death, 
                      aphid_dispersal#, 
                      #aphid_reproduction
                      )
    ...
    events <- c("i_plant_recovery", 
                #"aphid_death", 
                "aphid_dispersal"#, 
                #"aphid_reproduction"
                )
    
    event_rates <- c(i_plant_recovery, 
                     #aphid_death, 
                     aphid_dispersal#, 
                     #aphid_reproduction
                     )
    ...
    if (current_event == "i_plant_recovery") {
      ...
      if (I >= parms["H"]) {
        feeding <- T # no more plants to infect, so skip dispersal cycle
      }
      else {
        feeding <- F
      }
      
      print("aphid dispersal")
      print(paste("I=", I))
      S <- parms["H"] - I
      i_hat <- (v*I)/(S + v*I)
      ...
      while (feeding == F) {
        ...
        if (state == "flight") {
          print("flight")
          print(transition_matrix["flight",])
...
# define parameters
parms <- c(
  gamma = 3, #20/3, # rate of recovery/death of I plants
  ...
  p = 0, #0.2, # aphid emigration/death rate per journey between plants
...
plot(sims$time, sims$median, ylim = c(0, parms["H"]))
